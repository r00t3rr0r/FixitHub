...
if [ -z "$AUTH_TOKEN" ]; then
  ...
  echo -e "\n${YELLOW}Trying alternative credentials...${NC}"
  LOGIN_RESPONSE=$(curl -s -X POST "$BASE_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{
  ...
  if [ -z "$AUTH_TOKEN" ]; then
    echo -e "${RED}❌ Authentication failed with all credentials${NC}"
    exit 1
  fi
fi
...
if [ -z "$ORDER_ID" ]; then
  ...
  echo "Response excerpt: $(echo $ORDERS_RESPONSE | head -c 200)"
  echo -e "\n${YELLOW}Test cannot proceed without existing orders${NC}"
  exit 1
fi
...
if [ -z "$WORKFLOW_ID" ]; then
  ...
  echo "This is expected if no workflows match the order's device type"
  echo "Test will continue but workflow assignment cannot be tested"
  exit 0
fi
