...
curl -s -X POST "$BASE_URL/seed/admin" > /dev/null
...
if [ -z "$WORKFLOW_ID" ]; then
  echo -e "${YELLOW}⚠️  No suggested workflows found${NC}"
  exit 0
fi
...
# Step 5: Get the full order to extract actual workflow _id (not template _id)
echo -e "\n${BLUE}📝 Step 5: Getting assigned workflow ID...${NC}"
FULL_ORDER=$(curl -s -X GET "$BASE_URL/admin/orders/$ORDER_ID" \
  -H "Authorization: Bearer $AUTH_TOKEN")

# Extract the actual workflow instance _id (not the workflowTemplateId)
ASSIGNED_WORKFLOW_ID=$(echo $FULL_ORDER | python3 -c "
import json, sys
try:
    data = json.load(sys.stdin)
    order = data.get('order', {})
    if order.get('workflows') and len(order['workflows']) > 0:
        # Get the last added workflow (most recent)
        wf = order['workflows'][-1]
        if '_id' in wf:
            print(wf['_id'])
except:
    pass
...
echo -e "${GREEN}✅ Found assigned workflow ID: $ASSIGNED_WORKFLOW_ID${NC}"
...
VERIFY_ORDER=$(curl -s -X GET "$BASE_URL/admin/orders/$ORDER_ID" \
  -H "Authorization: Bearer $AUTH_TOKEN")

DELETED_FOUND=$(echo $VERIFY_ORDER | grep "$ASSIGNED_WORKFLOW_ID")
if [ -n "$DELETED_FOUND" ]; then
  echo -e "${RED}❌ Deleted workflow still exists!${NC}"
  exit 1
fi

echo -e "${GREEN}✅ Deletion verified - workflow no longer in order${NC}"

echo -e "\n═══════════════════════════════════════════════════════════════"
echo -e "\n${GREEN}✅ All tests passed! Multiple workflow feature is working!${NC}"
echo -e "\n${YELLOW}📋 Features Tested:${NC}"
echo -e "   ✓ Admin authentication"
echo -e "   ✓ Workflow assignment to order"
echo -e "   ✓ Multiple workflows support (can assign multiple)"
echo -e "   ✓ Workflow deletion from order"
echo -e "   ✓ Workflow list verification after deletion"
echo -e "\n${YELLOW}📋 Backend Endpoints Verified:${NC}"
echo -e "   ✓ POST /api/admin/orders/:orderId/workflows"
echo -e "   ✓ GET /api/admin/orders/:orderId/workflows"
echo -e "   ✓ DELETE /api/admin/orders/:orderId/workflows/:workflowId"
echo -e "\n"
